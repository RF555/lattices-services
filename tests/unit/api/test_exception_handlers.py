...
class TestExceptionHandlers:
    """Tests for API exception handler serialization."""

    @pytest.mark.asyncio
    async def test_app_exception_returns_error_code_and_message(self):
        """AppException is serialized with error_code, message, details."""
    ...
    @pytest.mark.asyncio
    async def test_http_exception_returns_standard_format(self):
        """Starlette HTTPException is serialized consistently."""
    ...
    async def test_validation_error_returns_field_details(self):
        """RequestValidationError returns field-level error details."""
    ...
    async def test_unhandled_exception_returns_500(self):
        """Unexpected exceptions return 500 with INTERNAL_ERROR code.

        Calls the global exception handler directly to avoid middleware
        wrapping exceptions in ExceptionGroup.
        """
        import json
