...
down_revision: str | Sequence[str] | None = "bc8a2f402d9d"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('pending', 'accepted', 'expired', 'revoked')", name="ck_invitations_status"
        ),
    ...
    op.create_index(
        "ix_invitations_workspace_email", "invitations", ["workspace_id", "email"], unique=False
    )
